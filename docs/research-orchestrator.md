...
`POST /research-runs/manual-import` seeds the same workflow from a Deep Research
prompt and report that were run manually in ChatGPT. The imported report is
stored as Deep Research attempt 1 with `source="manual_upload"`,
`model="chatgpt-deep-research-manual"`, `status="completed"`, no response id,
zero tool calls, and no Deep Research cost event.

The orchestrator still builds and stores an `ObjectiveContract` and
`ResearchItem` rows from the submitted prompt, but it saves the submitted prompt
as both `user_prompt` and `optimized_prompt`. The optimized prompt generated by
the contract builder is not persisted for manual imports.

Manual import writes `prompts/manual_import_prompt.txt`,
`reports/report_attempt_001.md`, and a metadata-only
`raw-responses/manual_import_001.json`. Raw prompt/report bodies are not stored
in the idempotency record; only hashes, lengths, filenames, content types, gate
settings, options, and import time are stored there.
...
requires `RESEARCH_POLLER_ENABLED=true`; with the poller disabled, the run can
remain `reviewing` until an operator intervenes through the normal review or
